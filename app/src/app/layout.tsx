        ...
        {!isAuthPage && (
          <aside className="hidden lg:flex w-64 bg-white m-4 mr-0 rounded-2xl border border-slate-200 flex-col shrink-0 shadow-sm overflow-hidden text-slate-700">
            ...
            <nav className="flex-1 px-4 mt-4 space-y-1">
              ...
              <Link href="/general-receipts" className="flex items-center gap-3 px-4 py-3 text-slate-500 hover:bg-slate-50 hover:text-slate-900 transition rounded-md">
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
                <span className="text-sm">General Receipts</span>
              </Link>
            ...
            <div className="p-4 border-t border-slate-100 flex flex-col gap-4">
              ...
              <div className="flex items-center gap-3 p-3 bg-slate-50 rounded-xl border border-slate-100">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[10px] text-slate-500 truncate">{session?.role === 'PRINCIPAL' ? 'Principal / Admin' : session?.role === 'MANAGEMENT' ? 'Management' : 'Internal User'}</p>
                </div>
              </div>
            </div>
          </aside>
